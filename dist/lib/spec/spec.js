...
function validateSpec(func, spec) {
    if (!func || !spec || !object$(spec) || not(string$(func))) {
        throw new Error(func && string$(func) ? `Malformed Spec in function: ${func}` : `Malformed Spec, missing function label`);
    }
}
function normaliseInputs(func, spec) {
    if (object$(func)) {
        spec = func.spec;
        func = func.func;
    }
...
export function enableSpecs(enabled$) {
    isEnabled = enabled$;
}
